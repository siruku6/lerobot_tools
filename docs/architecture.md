...
- [1. データ層](#1-データ層)
- [2. venv について](#2-venv-について)
- [3. データ構築と学習を分けない理由](#3-データ構築と学習を分けない理由)
- [4. 構成](#4-構成)
- [5. バージョンを変える](#5-バージョンを変える)
- [6. 直すときの決まりごと](#6-直すときの決まりごと)
...
[README.md の 3.1 データ層](../README.md#31-データ層) を参照。ここでは、そこに含めていない
実装レベルの話を補足する。
...
### python の版が違う理由は image ごとに別

- `eval` の 3.10 は**採点環境がそうだから**で、LIBERO の都合ではない
  （LIBERO 一式は 3.12 でも入り、import も通ることを実測した）
- `datagen` と `train` の 3.12 は **lerobot v0.6.0 の要求**で、動かせない

## 4. 構成

```
versions.env            データ image を決める git の ref。**python の版は書かない**
...
envs/                   **環境を作るものは全部ここ**
  ...
  eval/                 **採点環境の再現 image だけが使う**
    Dockerfile
    requirements.txt    版はここ。採点環境と同じく pip で入れる
    steps/venv.sh
...
**環境を作るものは `envs/` の下に閉じてある。** リポジトリ直下の `scripts/` は、
rollout や変換、学習といった**環境の上で走る処理**のために空けてある。
...
版が image ごとに違うのは、それぞれ理由がある。

- `eval` の 3.10 は**採点環境がそうだから**である。LIBERO の都合ではない
  （LIBERO 一式は 3.12 でも入り、import も通ることを実測した）
- `datagen` と `train` の 3.12 は **lerobot v0.6.0 の要求**で、動かせない
- `datagen` の numpy 1.26.4 は、**作ったデータを評価が読み直すから**である。
  numpy 2 で dtype の昇格規則（NEP 50）が変わり、同じコードが違う値を返しうる
- `train` の numpy 2.2.6 は、**これまで学習に使ってきた版**である

## 5. バージョンを変える

**python のパッケージ版は、それを使う image の依存定義ファイルを直す。**
...
`versions.env` には python のパッケージ版を書かない。**版は image ごとに違う**ので、
共通の場所に置くと `NUMPY_VERSION_DATAGEN` のような名前が増えていき、
どれがどこで使われるのか分からなくなる。

`versions.env` が持つのは、データ image を決める git の ref 3 つだけである。
これらは python のパッケージ版ではなく、pyproject では表現できない。
...
このファイルへの変更をそのままコミットすること。torch / torchcodec / torchvision の
組み合わせを崩してはいけない理由は
...
| # | 決まり | 破るとどうなるか |
|---|---|---|
| 1 | 版はそれを使う image の依存定義ファイルに書く。同じ版を 2 箇所に書かない | 片方だけ直して気付かず、実行時に落ちる |
| 2 | step は互いを取り込まない（`lib/` を除く） | 1 工程だけ流し直せなくなる |
| 3 | step は自分の事後条件を検査し、満たしていれば何もしない | 途中で落ちると頭から流し直しになる |
| 4 | Dockerfile は step を呼ぶだけ。判断を書かない | Docker が使えない環境向けに手順を書き直す羽目になる |

step の先頭には必ず「入力・出力・事後条件」の 3 行がある。単独で流してよいかは、それを読めば分かる。

規約 1 について補足する。以前は「版は `versions.env` にしか書かない」としていたが、
**実態と違っていた**。pyproject に実値が書かれ、`versions.env` はその複製を持って
事後条件の検査にだけ使っていた。同じ値が 2 箇所にある状態だったので、規約のほうを
実態に合わせた。検査は残してあるが、**依存定義ファイルの `==` 宣言と、実際に
入った版を突き合わせる**形にしてある。複製を作らずに検査だけ残せる。
